...
function sys_clean(){
	...
	sed -i '/iptables -A INPUT -p tcp --dport 55789 -j ACCEPT/d' /etc/rc.d/rc.local
	sed -i '/iptables -A INPUT -p udp --dport 55789 -j ACCEPT/d' /etc/rc.d/rc.local
...
iptables -A INPUT -p tcp --dport 55789 -j ACCEPT
iptables -A INPUT -p udp --dport 55789 -j ACCEPT
